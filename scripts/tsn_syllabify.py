...
class Phoneset(object):
    ...
    def is_syllabic(self, phonename):
        return "syllabic" in self.phones[phonename] and "consonant" in self.phones[phonename]
    ...
    def is_valid_CC(self, cluster, consider_foreign=True):
        ...
        if cluster[1] == "w" and any(isf(cluster[0]) for isf in [self.is_plosivelike, self.is_fricative, self.is_nasal, self.is_approximant, self.is_trill]):
            ...
            return True
        elif consider_foreign and cluster in self.foreign_CC_onsets:
        ...
        def breakcluster(cluster):
            if not cluster:
                ...
                if self.is_syllabic(cluster[0]):
                    ...
                    bounds.append(ci + 1)
                    return
                if cluster in self.foreign_CC_cluster_1split:
                ...
                if self.is_syllabic(cluster[0]):
                    ...
                    bounds.append(ci + 1)
                    return
                if cluster in self.foreign_CCC_onsets:
                ...
                if cluster[1:] in self.foreign_CC_onsets:
